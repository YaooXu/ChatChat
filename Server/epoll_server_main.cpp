...
void *hand_requset(void *arg) {
    ...
    if ((len = recv(confd, buf, sizeof(buf), 0)) == 0) {
        ...
        // 从红黑树删除
        epoll_ctl(epfd, EPOLL_CTL_DEL, pUser_connect_info->user_fd, NULL);
        pthread_exit(NULL);
    } else if (len == -1) {
        // TODO: 异常解决
        return NULL;
...
int main(int argc, char const *argv[]) {
    ...
    if (0 !=
        bind(listen_sockfd, (const sockaddr *)&server_addr, server_addr_len)) {
        perror("Bind error");
        return 1;
    }
    ...
    ev.events = EPOLLIN;  // 监听写事件
    ev.data.fd = listen_sockfd;

    epoll_ctl(epfd, EPOLL_CTL_ADD, listen_sockfd, &ev);
        ...
        for (int i = 0; i < ret; i++) {
            ...
            if (fd == listen_sockfd) {
                int connect_sockfd = accept(
                    listen_sockfd, (sockaddr *)&client_addr, &client_addr_len);
                if (connect_sockfd == -1) {
                ...
                User_connect_info *new_user = new User_connect_info();
                // 客户端的部分信息，此时还未登录
                strcpy(new_user->ipaddr, inet_ntoa(client_addr.sin_addr));
                new_user->user_fd = connect_sockfd;

                struct epoll_event tmp;
                tmp.events = EPOLLIN;
                tmp.data.ptr = new_user;
                ...
                User_connect_info *pUser_connect_info =
                    (User_connect_info *)all[i].data.ptr;

                pthread_t tid;
                // 给每个用户的操作创建一个线程
                pthread_create(&tid, NULL, hand_requset, pUser_connect_info);
            }
        }
    }
    return 0;
